...
describe('main Factory', () => {
  ...
  describe('[REST API]', () => {
    it('should generate appropriate files ', async () => {
      ...
      const tree = await runner.runSchematicAsync('main', options).toPromise();
    ...
    it("should keep underscores in main's path and file name", async () => {
      const options: MainOptions = {
        name: '_users',
        type: 'rest',
      };
    ...
    describe('when "crud" option is not enabled', () => {
      it('should generate appropriate files (without dtos)', async () => {
        ...
        const tree = await runner
          .runSchematicAsync('main', options)
          .toPromise();
        const files = tree.files;
        expect(files).toEqual([
          '/users/users.controller.spec.ts',
  ...
  describe('[REST API]', () => {
    ...
    beforeAll(async () => {
      tree = await runner.runSchematicAsync('main', options).toPromise();
    });
