...
export function main(options: MainOptions): Rule {
...
function transform(options: MainOptions): MainOptions {
  const target: MainOptions = Object.assign({}, options);
  if (!target.name) {
  ...
  if (target.language === 'js') {
    throw new Error(
      'The "main" schematic does not support JavaScript language (only TypeScript is supported).',
    );
  }
...
function generate(options: MainOptions): Source {
...
function addDeclarationToModule(options: MainOptions): Rule {
  return (tree: Tree) => {
...
function addMappedTypesDependencyIfApplies(options: MainOptions): Rule {
